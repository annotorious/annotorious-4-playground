...
export const createPointerHandling = <E>(
  ...
  const startDrawing = (event: PointerEvent) => {
    ...
    // Only 'drag' mode needs the viewer's own pan/zoom out of the way: it
    // draws via one continuous press-drag-release gesture, which would
    // otherwise double as a pan. 'click' mode is the opposite by design -
    // each tool places a vertex/corner on a qualifying pointerup and
    // otherwise ignores pointer movement entirely (see drawing-tool.ts's
    // doc and each tool's CLICK_TIMEOUT_MS-gated onPointerUp, which already
    // treats a slow/moved pointerup as "not a genuine click, e.g. a pan" and
    // ignores it) - specifically so the user can freely pan/zoom between
    // clicks to reposition the view before placing the next point.
    if (drawingMode === 'drag') viewer.setMouseNavEnabled(false);
  ...
  // onUpdateViewport below). Not just a screen position: eventToWorld needs
  // the original event.
  ...
  // actually moving *inside* `viewer.element` - it never fires again once
  // the pointer leaves, so a shape hovered right before the cursor exits
  // the canvas (or a drag/gesture elsewhere steals pointer capture) stays
  ...
  viewer.element.addEventListener('pointerleave', onPointerLeave);
  viewer.element.addEventListener('pointercancel', onPointerLeave);
  ...
  const destroy = () => {
    ...
    viewer.element.removeEventListener('pointerleave', onPointerLeave);
    viewer.element.removeEventListener('pointercancel', onPointerLeave);
