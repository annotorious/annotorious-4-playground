...
import type { Update } from '../state/store-observer';
...
const getChangedBodies = (oldValue: Annotation, newValue: Annotation) =>
  newValue.bodies
    ...
    .filter((pair): pair is { newBody: Annotation['bodies'][number], oldBody: Annotation['bodies'][number] } => Boolean(pair.oldBody));

const hasTargetChanged = (oldValue: Annotation, newValue: Annotation) =>
  !dequal(oldValue.target, newValue.target);

/** Computes a structured diff between two versions of the same annotation **/
export const diffAnnotations = <T extends Annotation = Annotation>(oldValue: T, newValue: T): Update<T> => {
  ...
  return {
    ...
    targetUpdated: hasTargetChanged(oldValue, newValue)
      ? { oldTarget: oldValue.target, newTarget: newValue.target } : undefined
  } as Update<T>;
}
